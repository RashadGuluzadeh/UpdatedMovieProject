...
const MovieItem = ({title, poster_path, release_date, id, disabled}) => {
  ...
  return (
      <div className=" relative flex flex-col justify-center item-center text-center p-4 bg-[#151312] shadow-lg backdrop-4">
        <button disabled={disabled} onClick={() => dispatch(addListMovie(id))} type='button' className='text-white w-16 h-16 bg-blue-600 ease-in-out text-3xl px-5 py-3 absolute top-[-15px] right-[-15px] rounded-[50px]'>{!disabled ? "+" : "✓"}</button>
